...
namespace BussinessLayer.Service
{
    public class VaccinationEventService : IVaccinationEventService
    {
        ...
        public async Task<bool> ProcessEmailReplyAsync(string fromEmail, string subject, string body)
        {
            try
            {
                // Parse email content to extract response
                var response = ParseEmailResponse(subject, body);
                if (response == null)
                    return false;

                // Find student by parent email
                var students = await _vaccinationEventRepository.GetStudentsForEventAsync(response.EventId);
                var student = students.FirstOrDefault(s => s.Parent?.Email == fromEmail);
                
                if (student == null)
                    return false;

                // Process the response
                var dto = new ParentVaccinationResponseDTO
                {
                    ParentId = student.Parentid,
                    StudentId = student.Studentid,
                    VaccinationEventId = response.EventId,
                    WillAttend = response.WillAttend,
                    ReasonForDecline = response.ReasonForDecline,
                    ParentConsent = response.ParentConsent
                };

                return await ProcessParentResponseAsync(dto);
            }
            catch (Exception ex)
            {
                // Log the exception
        ...
        private EmailResponse? ParseEmailResponse(string subject, string body)
        {
            try
            {
                ...
                // Extract event ID from subject or body
                var eventIdMatch = System.Text.RegularExpressions.Regex.Match(subject + " " + body, @"event[:\s]*(\d+)", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                if (eventIdMatch.Success)
                {
                    response.EventId = int.Parse(eventIdMatch.Groups[1].Value);
                }
                ...
                var positiveKeywords = new[] { "đồng ý", "có", "yes", "ok", "agree", "accept", "thích", "muốn" };
                var negativeKeywords = new[] { "không", "no", "decline", "refuse", "từ chối", "không muốn" };

                response.WillAttend = positiveKeywords.Any(keyword => lowerBody.Contains(keyword) || lowerSubject.Contains(keyword));
                
                if (!response.WillAttend)
                {
                    response.WillAttend = !negativeKeywords.Any(keyword => lowerBody.Contains(keyword) || lowerSubject.Contains(keyword));
                }

                // Extract reason for decline
                if (!response.WillAttend)
                {
                    var reasonLines = lines.Where(line => 
                        line.ToLower().Contains("lý do") || 
                        line.ToLower().Contains("reason") || 
                        line.ToLower().Contains("vì") ||
                        line.ToLower().Contains("do"))
                        .ToList();

                    if (reasonLines.Any())
                    {
                        response.ReasonForDecline = string.Join(" ", reasonLines);
                    }
                }

                response.ParentConsent = response.WillAttend; // Assume consent if they agree

                return response.EventId > 0 ? response : null;
            }
            catch
            {
                return null;
